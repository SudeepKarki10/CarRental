...
const SearchForm = React.forwardRef<HTMLDivElement, SearchFormProps>(
  ({ selectedCar }, ref) => {
    const cars = [
      "Tata Nexon",
      "Maruti Suzuki Swift",
    ...
    // Update carType when selectedCar changes
    useEffect(() => {
      if (selectedCar) {
    ...
    return (
      <div ref={ref} className="Booking-Form bg-grey-100 py-10">
        {message ? (
          <div className="mx-5 md:mx-20 md:mr-20 h-12 rounded-md bg-green-400 flex justify-start items-center">
            <p className="text-lg text-zinc-600 font-semibold block">
              {message}
            </p>
          </div>
        ) : null}

        <form className="bg-white mx-5 md:mx-20 grid grid-cols-1 ssm:grid-cols-2 lg:grid-cols-3 gap-4 ssm:gap-10 shadow-lg p-10">
